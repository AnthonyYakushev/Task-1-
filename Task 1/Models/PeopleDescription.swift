...
struct PeopleDescription {
    ...
    static func getArrayFromPeople(_ people: People) -> [PeopleDescription] {
        ...
        if let films = people.films, films.isEmpty == false {
            peopleDescriptionArray.append(PeopleDescription(title: "Films:", subtitle: films.joined(separator: "\n")))
        }
        
        if let species = people.species, species.isEmpty == false {
            peopleDescriptionArray.append(PeopleDescription(title: "Species:", subtitle: species.joined(separator: "\n")))
        }
        
        if let vehicles = people.vehicles, vehicles.isEmpty == false {
            peopleDescriptionArray.append(PeopleDescription(title: "Vehicles:", subtitle: vehicles.joined(separator: "\n")))
        }
        
        if let starships = people.starships, starships.isEmpty == false {
            peopleDescriptionArray.append(PeopleDescription(title: "Starships:", subtitle: starships.joined(separator: "\n")))
        }
